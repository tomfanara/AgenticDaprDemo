...
using static Workflow.API.Models.TaskChainingModels;
    ...
    public class GroupChatWorkflowCommandHandler(DaprWorkflowClient daprWorkflowClient) : IRequestHandler<GroupChatHandlerRequest, Chat>
    {
        ...
        public async Task<Chat> Handle(GroupChatHandlerRequest request, CancellationToken cancellationToken)
        {           
        ...
        var response = await this.daprWorkflowClient.ScheduleNewWorkflowAsync(           
            ...
            Console.WriteLine($"{nameof(MonitorWorkflow)} (ID = {workflowId}) started successfully");
